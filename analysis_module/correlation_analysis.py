...
file_path = "D:\\PD\\paper\\2EP\\TLE_mainly.csv"
...
with open('D:\\PD\\paper\\2EP\\TLE_results\\correlation_results_ROIs_sig.txt', 'w') as file:
    ...
    for ROI in ROIs:
        for subtype in ['Subtype1_Freq', 'Subtype2_Freq', 'Subtype3_Freq', 'Subtype4_Freq']:
            ...
            corr, p_value = pearsonr(data[ROI], data[subtype])
            # Extract the correlation coefficient and p-value from the DataFrame
            #corr = result['r'].values[0]
            #p_value = result['p-val'].values[0]
            
            # Format the output string
            #output_str = f"Correlation between {ROI} and {subtype}: partial r = {corr:.2f}, p-value = {p_value:.3f}\n"
            if p_value < 0.05 :
